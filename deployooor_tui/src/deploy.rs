...
impl ConstructorArguments {
    fn new(types: &[&str], values: &[&str]) -> Self {
        let (sol_types, values) = types.iter().zip(values).intersperse((&",", &",")).fold(
...
pub mod tests {
    ...
    async fn deploy_solidity() {
        ...
        let contract =
            Contract::from_str("/users/crypdoughdoteth/dev/active/vyperdeployooor/src-tauri/src/soliditylayout/contracts/storage.sol").unwrap();
