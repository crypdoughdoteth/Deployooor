...
pub mod tests {
    ...
    async fn deploy() {
        use std::path::Path;
        use vyper_rs::{vyper, vyper::Vyper};

        let mut contract = vyper!("./multisig.vy");
        let abi = contract.get_abi().unwrap().to_string();
        contract.compile().unwrap();

        let deployment = ContractDeployment {
            provider: "https://eth-sepolia.g.alchemy.com/v2/EKzkmO53kmo7_hUc2nileb1bZxNvz_aV",
            args: Box::new([
                ConstructorArg {
                    ty: EthType::Uint,
                    value: Box::new(["1".to_owned()]),
            ...
            abi: &abi,
            initcode: &contract.bytecode.unwrap(),
            private_key: "0x1bc8d86cea7111253126e0675756a470d95ce912b529f7121412be63a8f877e3",
        };
        // address: 0x21C5d020E95f6361eb81b64d0d1Fd1a0c4A2c510
        // pk 0x1bc8d86cea7111253126e0675756a470d95ce912b529f7121412be63a8f877e3
        deploy_contract(deployment).await.unwrap();
    }
}
